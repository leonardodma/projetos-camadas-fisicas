...
def get_handshake(size, porta):
    ...
    while continua:
        while tentativas <= 4:
            time.sleep(1)
            print('Inatividade percebida...')
            print(f'Tentando novamente... tentativas[{tentativas}/4]')
            response = porta.rx.getOnTime(size, 5)
...
def create_log(get_or_send, TYPE, size, ID=None, n_packages=None, CRC=None):
    ...
    if ID == None and n_packages == None:
        ...
        log = dt_string+' | '+get_or_send+' | '+str(TYPE)+' | '+str(size)+' | '+str(ID)+' | '+str(n_packages)+' | '+str(CRC)

    return log


def write_client_log(log):
     with open("logs/Client3.txt", "a") as f:
        f.write(f"{log}\n")


def write_server_log(log):
    with open("logs/Server3.txt", "a") as f:
     f.write(f"{log}\n")