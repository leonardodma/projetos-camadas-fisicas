...
from crcmod.crcmod import mkCrcFun
from utils import *
from crcmod import *
...
class Head():
    def __init__(self, h0): # São fixos
        ...
        self.h1 = client_id
        self.h2 = server_id
        self.crc = (0).to_bytes(2, 'big')

    def messegeType(self):
        return byte_to_int(self.h0)
    
    def createHead(self):
        return self.h0 + self.h1 + self.h2 + self.h3 + self.h4 + self.h5 + self.h6 + self.h7 + self.crc
            ...
            if self.messegeType() == 3:
                ...
                for i in range(0, len(self.message), 114):
                    if len(self.message) - i >= 114:
                        ...
                        self.crc = crc16(payload).to_bytes(2, "big")
